...
11. Add `security` job to `.github/workflows/ci.yml` — runs `npm audit --audit-level=high`, fails build on high/critical vulnerabilities, runs in parallel with unit tests after build passes
12. Update branch protection to require `unit-tests` and `security` status checks in addition to `ci` — as new CI jobs are added, they should also be required before merge
